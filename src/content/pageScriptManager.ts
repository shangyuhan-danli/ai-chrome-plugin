// PageScript 管理器 - 安全地执行动态加载的 JS 脚本
// 使用 CSP 和沙箱环境确保安全性
//
// ⚠️ 安全警告：当前沙箱实现存在安全问题，功能已禁用
// 问题：allow-scripts + allow-same-origin 组合会使沙箱失效
// TODO: 需要重新设计安全方案后再启用
...
// 保留旧类型别名以兼容
export type Skill = PageScript

/**
 * PageScript 执行器 - 在沙箱环境中执行脚本
 * ⚠️ 当前功能已禁用，等待安全方案重新设计
 */
class PageScriptManager {
  private scripts: Map<string, PageScript> = new Map()
  private sandboxIframe: HTMLIFrameElement | null = null

  /**
   * 注册 PageScript
   */
  registerScript(script: PageScript): void {
    // 验证脚本安全性
    if (!this.validateScript(script)) {
      throw new Error(`PageScript "${script.name}" 验证失败`)
    }

    this.scripts.set(script.id, script)
    console.log(`[PageScriptManager] 注册 PageScript: ${script.name} (${script.id})`)
  }

  /**
   * 验证 PageScript 安全性
   */
  private validateScript(script: PageScript): boolean {
    // 1. 检查是否包含危险操作
    ...
    for (const pattern of dangerousPatterns) {
      if (pattern.test(script.script)) {
        console.warn(`[PageScriptManager] 检测到危险操作: ${pattern}`)
        return false
      }
    }
    ...
    if (script.script.length > 100000) {
      console.warn('[PageScriptManager] 脚本过大')
      return false
    }
   ...
   * ⚠️ 功能已禁用 - 安全问题待解决
   */
  async executeScript(scriptId: string, context: {
    url: string
    pageTitle: string
    elements?: any[]
    [key: string]: any
    ...
    return {
      success: false,
      error: 'PageScript 功能已禁用，等待安全方案重新设计'
    }

    /* 原始实现已注释 - 存在安全问题
    const script = this.scripts.get(scriptId)
    if (!script) {
      return { success: false, error: `PageScript "${scriptId}" 未找到` }
    }

    // 检查 URL 匹配
    if (script.matchPatterns && script.matchPatterns.length > 0) {
      const matches = script.matchPatterns.some(pattern => {
        try {
      ...
      if (!matches) {
        return { success: false, error: `当前页面不匹配 PageScript 的 URL 模式` }
      }
    }

    try {
      // 在沙箱中执行
      const result = await this.executeInSandbox(script.script, context)
      return { success: true, result }
    } catch (error) {
      console.error(`[PageScriptManager] 执行 PageScript "${scriptId}" 失败:`, error)
  ...
  async executeSkill(skillId: string, context: {
    ...
    return this.executeScript(skillId, context)
  }
   ...
   *
   * ⚠️ 安全问题：
   * - allow-scripts + allow-same-origin 组合会使沙箱失效
   * - 脚本可以通过 parent.document 直接访问父页面 DOM
   * - 危险模式检测可被绕过（如 window['ev'+'al']）
   */
  private async executeInSandbox(_script: string, _context: Record<string, any>): Promise<any> {
    throw new Error('沙箱执行功能已禁用，等待安全方案重新设计')
  ...
  /**
   * 查找匹配当前页面的 PageScripts
   */
  findMatchingScripts(url: string): PageScript[] {
    return Array.from(this.scripts.values()).filter(script => {
      if (!script.matchPatterns || script.matchPatterns.length === 0) {
        return true  // 通用 PageScript
      }
  ...
  // 保留旧方法名以兼容
  findMatchingSkills(url: string): PageScript[] {
    return this.findMatchingScripts(url)
  }

  /**
   * 获取所有 PageScripts
   */
  getAllScripts(): PageScript[] {
    return Array.from(this.scripts.values())
  }

  // 保留旧方法名以兼容
  getAllSkills(): PageScript[] {
   ...
   * 移除 PageScript
   */
  removeScript(scriptId: string): boolean {
    return this.scripts.delete(scriptId)
  }

  // 保留旧方法名以兼容
  removeSkill(skillId: string): boolean {
    return this.removeScript(skillId)
  }
}

// 导出单例
export const pageScriptManager = new PageScriptManager()

// 保留旧导出名以兼容
export const skillManager = pageScriptManager
